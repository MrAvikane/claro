...
def display():
    ...
    if request.method == 'POST':
        ...
        if selection == 'IMEI':
            cur1.execute('''SELECT MATV_SERIE FROM SISCAD_MATERIAL W WHERE W.MATV_NRO_PEDIDO in (SELECT 
            x.pedv_nro_pedido from siscad_pedido x where x.pedv_punto_venta in ('0004850257') and x.pedv_estado = 
            '02') AND w.MATV_ESTADO = 'L' AND w.MATV_TIPO_MAT = '01' -- 01 CHIPS - 02 EQUIPOS AND w.matv_serie IN (
            select LTRIM(o.SERIC_CODSERIE, '0') FROM SSAPT_SERIE@DBL_MSSAP o where o.INTEV_CODINTERLOCUTOR = 
            '0004850257')''')
            data = cur1.fetchall()
            ...
            cur1.execute('''SELECT MATV_SERIE FROM SISCAD_MATERIAL W WHERE W.MATV_NRO_PEDIDO in (SELECT 
            x.pedv_nro_pedido from siscad_pedido x where x.pedv_punto_venta in ('0004850257') and x.pedv_estado = 
            '02') AND w.MATV_ESTADO = 'L' AND w.MATV_TIPO_MAT = '02' -- 01 CHIPS - 02 EQUIPOS AND w.matv_serie IN (
            select LTRIM(o.SERIC_CODSERIE, '0') FROM SSAPT_SERIE@DBL_MSSAP o where o.INTEV_CODINTERLOCUTOR = 
            '0004850257')''')
            data = cur1.fetchall()
            data = [i for sub in data for i in sub]
